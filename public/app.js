...
const pages = {
  ...
  calendar: {
    ...
    render() {
      ...
      ])}<section class="two-col strong-left"><article class="panel calendar"><div class="calendar-head"><button type="button" data-calendar-nav="-1">Previous Month</button><h2>${monthTitle}</h2><button type="button" data-calendar-nav="1">Next Month</button></div><div class="weekdays">${["Mon","Tue","Wed","Thu","Fri","Sat","Sun"].map(day => `<b>${day}</b>`).join("")}</div><div class="days">${calendarGrid(visibleMonth, monthEvents)}</div></article><article class="panel">${table(["Date", "Activity", "Type"], monthEvents.map(event => `<tr><td><b>${displayDate(eventDate(event))}</b></td><td><b>${event.company || event.title || event.meeting || "Activity"}</b><br><span>${event.meeting || event.title || "Next activity"}${state.user.role === "admin" && event.owner ? ` · ${event.owner}` : ""}</span></td><td>${event.type || "Activity"}</td></tr>`), "No records")}</article></section>`;
    }
  },
  reports: {
