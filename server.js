...
const LIVE_ARG_ADD_LEAD_FIELDS = [
  ...
  "Next action date", "Next Action", "Scope of Action", "First order date", "Est. monthly volume", "Product interest", "Tags", "Quotation ref",
...
function cleanContact(contact = {}, index = 0) {
  return {
    id: contact.id || `c-${Date.now()}-${index}`,
    name: String(contact.name || contact.contactPerson || "").trim(),
    title: String(contact.title || contact.primaryTitle || "").trim(),
...
function normalizeLeadContacts(lead) {
  ...
  const fallback = {
    id: "default",
    name: lead.contactPerson || lead.name || "Default Contact",
    title: lead.primaryTitle || "",
    phone: lead.phone || "",
    email: lead.email || "",
    isDefault: true
  };
  let contacts = (existing.length ? existing : [fallback])
    .map(cleanContact)
    .filter(contact => contact.name || contact.phone || contact.email || contact.title);
  if (!contacts.length) contacts = [cleanContact(fallback)];
  let defaultIndex = contacts.findIndex(contact => contact.isDefault);
  if (defaultIndex < 0) defaultIndex = 0;
  contacts = contacts.map((contact, index) => ({ ...contact, isDefault: index === defaultIndex }));
  const defaultContact = contacts[defaultIndex];
  lead.contacts = contacts;
  lead.contactPerson = defaultContact.name || lead.contactPerson || lead.name || "";
  lead.name = lead.contactPerson;
  lead.primaryTitle = defaultContact.title || lead.primaryTitle || "";
  lead.phone = defaultContact.phone || lead.phone || "";
  lead.email = defaultContact.email || lead.email || "";
  return contacts;
}
...
function normalizeCompanyRecord(lead, index = 0) {
  ...
  lead.quotationRef ||= "";
  lead.productRemarks ||= "";
  lead.nextAction ||= lead.nextActionType || lead.purpose || "";
  lead.purpose ||= [lead.nextActionType, lead.scopeOfAction].filter(Boolean).join(" - ") || lead.nextAction;
...
async function handleApi(req, res) {
  ...
  if (req.method === "POST" && url.pathname === "/api/leads") {
    ...
    const nextActionType = String(body.nextActionType || body["Next Action"] || "").trim();
    const scopeOfAction = String(body.scopeOfAction || body["Scope of Action"] || "").trim();
    const structuredNextAction = [nextActionType, scopeOfAction].filter(Boolean).join(" - ");
    const nextAction = String(body.nextAction || structuredNextAction || "Initial qualification").trim();
    const lead = {
      id: `l${leads.length + 1}`,
      ...
      nextAction,
      purpose: structuredNextAction || nextAction,
      notes: body.notes ? [body.notes] : [],
      contacts: [{
        id: "default",
        name: body.contactPerson || body["Contact person"] || body.name || "Default Contact",
        title: body.primaryTitle || body["Primary title"] || "",
        phone: body.phone || "",
        email: body.email || "",
        isDefault: true
      }]
